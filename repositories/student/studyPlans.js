...
async function solveSessionQuestion({
  ...
  const [qCheck] = await client.execute(
    `${checkSql} WHERE q.question_id = ? ${
      checkWhere.length ? " AND " + checkWhere.join(" AND ") : ""
    } LIMIT 1`,
    checkVals
  );
  if (!qCheck.length)
    return { success: false, message: "Question not in session scope" };
...
async function getTopicsBySubject({ moduleId, studentId, includeQuestions = true }) {
  ...
  const sql = `
    ...
    q AS (
      SELECT q.question_id, q.topic_id, q.difficulty_level
      FROM questions q
      JOIN act_topics t ON t.topic_id = q.topic_id
    ),
    q_agg AS (
      SELECT
        topic_id,
        ...
        SUM(CASE WHEN difficulty_level = 'hard'   THEN 1 ELSE 0 END) AS difficult_count
      FROM q
      GROUP BY topic_id
    ),
    f_agg AS (
      SELECT f.topic_id, COUNT(*) AS flashcards_count
      FROM flashcards f
    ...
    , last_solve AS (
      ...
      FROM (
        SELECT
          s.question_id,
          s.is_correct,
          ROW_NUMBER() OVER (PARTITION BY s.question_id ORDER BY s.created_at DESC) AS rn
        FROM solved_questions s
        WHERE s.student_id = ?
      ) x
      WHERE rn = 1
    ),
    marks AS (
      SELECT mcq.question_id
      FROM mark_category_question mcq
      JOIN student_mark_categories smc
    ...
    perq AS (
      SELECT
        ...
        q.question_id,
        q.difficulty_level,
        CASE WHEN ls.question_id IS NULL THEN 0 ELSE 1 END AS attempted,
        CASE WHEN ls.is_correct = '1' THEN 1 ELSE 0 END AS correct,
        CASE WHEN m.question_id IS NULL THEN 0 ELSE 1 END AS marked
      FROM q
      LEFT JOIN last_solve ls ON ls.question_id = q.question_id
      LEFT JOIN marks m      ON m.question_id  = q.question_id
    ),
    q_student AS (
      SELECT
        topic_id,
        SUM(attempted) AS attempted_count,
        SUM(correct) AS correct_count,
        SUM(CASE WHEN attempted = 1 AND correct = 0 THEN 1 ELSE 0 END) AS wrong_count,
        SUM(CASE WHEN attempted = 0 THEN 1 ELSE 0 END) AS unsolved_count,
        SUM(marked) AS marked_count,

        SUM(CASE WHEN correct = 1 AND difficulty_level = 'easy'   THEN 1 ELSE 0 END) AS correct_count_easy,
        SUM(CASE WHEN correct = 1 AND difficulty_level = 'medium' THEN 1 ELSE 0 END) AS correct_count_medium,
        SUM(CASE WHEN correct = 1 AND difficulty_level = 'hard'   THEN 1 ELSE 0 END) AS correct_count_hard,

        SUM(CASE WHEN attempted = 1 AND correct = 0 AND difficulty_level = 'easy'   THEN 1 ELSE 0 END) AS wrong_count_easy,
        SUM(CASE WHEN attempted = 1 AND correct = 0 AND difficulty_level = 'medium' THEN 1 ELSE 0 END) AS wrong_count_medium,
        SUM(CASE WHEN attempted = 1 AND correct = 0 AND difficulty_level = 'hard'   THEN 1 ELSE 0 END) AS wrong_count_hard,

        SUM(CASE WHEN attempted = 0 AND difficulty_level = 'easy'   THEN 1 ELSE 0 END) AS unused_count_easy,
        SUM(CASE WHEN attempted = 0 AND difficulty_level = 'medium' THEN 1 ELSE 0 END) AS unused_count_medium,
        SUM(CASE WHEN attempted = 0 AND difficulty_level = 'hard'   THEN 1 ELSE 0 END) AS unused_count_hard,
    ...
    SELECT
      ...
      su.unit_id,
      su.unit_name,
      COALESCE(qa.questions_count, 0)  AS questions_count,
      COALESCE(fa.flashcards_count, 0) AS flashcards_count,
      ...
      COALESCE(qs.unsolved_count, 0)   AS unsolved_count,
      COALESCE(qs.marked_count, 0)     AS marked_count,

      COALESCE(qs.correct_count_easy, 0)   AS correct_count_easy,
      COALESCE(qs.correct_count_medium, 0) AS correct_count_medium,
      COALESCE(qs.correct_count_hard, 0)   AS correct_count_hard,

      COALESCE(qs.wrong_count_easy, 0)     AS wrong_count_easy,
      COALESCE(qs.wrong_count_medium, 0)   AS wrong_count_medium,
      COALESCE(qs.wrong_count_hard, 0)     AS wrong_count_hard,

      COALESCE(qs.unused_count_easy, 0)    AS unused_count_easy,
      COALESCE(qs.unused_count_medium, 0)  AS unused_count_medium,
      COALESCE(qs.unused_count_hard, 0)    AS unused_count_hard,

      COALESCE(qs.marked_count_easy, 0)    AS marked_count_easy,
      ...
      0 AS correct_count,
      0 AS wrong_count,
      COALESCE(qa.questions_count, 0) AS unsolved_count,
      0 AS marked_count,
      ...
      COALESCE(qa.easy_count,   0) AS unused_count_easy,
      COALESCE(qa.medium_count, 0) AS unused_count_medium,
      COALESCE(qa.difficult_count, 0) AS unused_count_hard,
  ...
  // If we included questions_json, map it to a JS array named "questions"
  const data = rows.map((r) => {
    if (r.questions_json != null) {
      try {
        r.questions = JSON.parse(r.questions_json);
      } catch {
        r.questions = [];
      }
      delete r.questions_json;
    } else {
      r.questions = []; // keep same shape as your original
    }
    return r;
  });
