...
async function solveQuestion(req, res) {
  ...
  const question = await repo.solveQuestion({
    question_id,
    studentId,
    answer,
    qbank_id,
...
async function createDeck(req, res) {
  ...
  try {
    const deck_id = await repo.createDeck({
      studentId,
      qbank_id,
      question_id,
      deck_title,
...
async function listFlashcardsByDeck(req, res) {
  ...
  try {
    const cards = await repo.listFlashcardsByDeck({
      studentId,
      deck_id,
      mode,
      limit,
...
async function createFlashCard(req, res) {
  ...
  const {
    deck_id,
    student_flash_card_front: front,
    student_flash_card_back: back,
    tags,
    difficulty,
    question_id,
    qbank_id,
...
async function listDueFlashcards(req, res) {
  ...
  try {
    const cards = await repo.getFlashcardsByMode({
      studentId,
      limit,
      mode,
      deckId,
...
async function reviewFlashcard(req, res) {
  ...
  try {
    const result = await repo.reviewFlashcard({
      studentId,
      student_flash_card_id,
      quality,
      correct,
...
async function getOnDemandExams(req, res) {
  ...
  try {
    ...
    return responseBuilder.serverError(
      res,
      "Failed to retrieve on-demand exams"
    );
  }
}
...
async function submitExamAnswer(req, res) {
  ...
  const { attemptId, examQuestionId, answerText, selectedOptionId, timeSpent } =
    req.body;
